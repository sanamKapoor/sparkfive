...
export const getAssociatedCampaigns = (asset) => {
  ...
  if (entries.length > 0) return entries.map(([_, name]) => name).join(", ");
  else return "No Campaigns";
};
...
export const getAssetsFilters = ({
  ...
  Object.keys(userFilterObject).map((key) => {
    ...
    if (key.includes("custom-p")) {
      ...
      const index = key.split("custom-p")[1];
      if (
        userFilterObject[key] &&
        userFilterObject[key].length > 0 &&
        userFilterObject[`all-p${index}`] &&
        userFilterObject[`all-p${index}`] !== "none"
      ) {
