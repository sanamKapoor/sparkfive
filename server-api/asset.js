...
export default {
  ...
  uploadThumbnail: (formData, queryData = {}) =>
    axios.post(`${assetUrl}/upload/thumbnail?${querystring.encode(queryData)}`, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    }),
  ...
  copyAssets: ({ idList, folderId }, filters = {}) =>
    axios.post(`${assetUrl}/copy?${querystring.encode(filters)}`, { idList, folderId }),
  moveAssets: ({ idList, folderId }, filters = {}) =>
    axios.post(`${assetUrl}/move?${querystring.encode(filters)}`, { idList, folderId }),
  getRealUrl: (assetId) => axios.get(`${assetUrl}/${assetId}/real-url`),
  importAssets: (provider, assetData, queryData) =>
    axios.post(`${assetUrl}/import/${provider}?${querystring.encode(queryData)}`, assetData),
  updateMultiple: (updateData, filters = {}) => axios.patch(`${assetUrl}?${querystring.encode(filters)}`, updateData),
  updateMultipleAttributes: (updateData, filters = {}) =>
    axios.patch(`${assetUrl}/attributes?${querystring.encode(filters)}`, updateData),
  ...
  deleteMultipleAssets: ({ assetIds, filters = {} }) =>
    axios.delete(`${assetUrl}?${querystring.encode(filters)}`, { data: { assetIds } }),
  ...
  associate: (assetIds, queryData) =>
    axios.post(`${assetUrl}/associate?${querystring.encode(queryData)}`, { assetIds }),
