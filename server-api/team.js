...
export default {
  ...
  verifyDomain: (subdomain) =>
    axios.post(`${teamUrl}/verify-domain`, { subdomain }),
  patchTeam: (patchData) => axios.patch(teamUrl, patchData),
  getTeamMembers: () => axios.get(`${teamUrl}/members`),
  patchTeamMember: (id, updatedata) =>
    axios.patch(`${teamUrl}/members/${id}`, updatedata),
  ...
  saveAdvanceConfigurations: (data) =>
    axios.put(`${teamUrl}/saveAdvanceConfigurations`, data),
  ...
  addOcrCustomFields: (data) =>
    axios.post(`${teamUrl}/ocr-custom-fields`, data),
  removeOcrCustomFields: (id) =>
    axios.delete(`${teamUrl}/ocr-custom-fields/${id}`),
