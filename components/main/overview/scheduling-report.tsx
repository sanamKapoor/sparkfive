...
const SchedulingReport = () => {
  ...
  const filterAndCompute = (
    ...
    itemStatuses.forEach((status) => {
      const filteredCampaignsLength = campaignsData.filter(
        (campaign) =>
          campaign.status === status && isInDateRange(campaign.endDate)
      ).length;
      const filteredProjectsLength = projectsData.filter(
        (project) =>
          project.status === status && isInDateRange(project.publishDate)
      ).length;
      const filteredTasksLength = tasksData.filter(
        (task) => task.status === status && isInDateRange(task.endDate)
      ).length;
      const currentTotal =
        filteredCampaignsLength + filteredProjectsLength + filteredTasksLength;
    ...
    // Get percentages
    itemStatuses.forEach((status) => {
      totalsObj[status].percent =
        totalCount > 0
          ? ((totalsObj[status].total / totalCount) * 100).toFixed()
          : 0;
    });
  ...
  const Legend = ({ status }) => (
    <div className={styles.legend}>
      <span className={`${styles["status-dot"]} ${styles[status]}`}></span>
        ...
        <div className={styles.heading}>
          <ul className={styles.legends}>
            {itemStatuses.map((status, index) => (
              <Legend key={index} status={status} />
            ))}
          </ul>
          <div className={styles.month}>
            <Select
              ...
              options={availableMonths.map((monthDate) => ({
        ...
        <div className={styles.content}>
          <div className={styles["chart-wrapper"]}>
            <div className={styles.chart}>
              {data && <ChartWrapper chartObj={chartObj} data={data} />}
            </div>
          </div>
          <ul className={styles.totals}>
            {itemStatuses.map((status, index) => (
              <AggregatedValue key={index} status={status} />
            ))}
          </ul>
        </div>
      </div>
