...
const TasksList = ({
  ...
  const handleUserSelection = (user) => {
    if (user.id === tasks[visibleMemberIndex].users[0]?.id)
      replaceTaskAssigned(visibleMemberIndex, undefined);
  ...
  const handleClickOutsideMore = (event) => {
    ...
    if (
      wrapperRefMore.current &&
      !wrapperRefMore.current.contains(event.target) &&
      className !== "more-icon"
    ) {
  ...
  const handleClickOutsideDate = (event) => {
    const className = event.target.getAttribute("class");
    if (
      wrapperRefDate.current &&
      !wrapperRefDate.current.contains(event.target) &&
  ...
  const handleClickOutsideMember = (event) => {
    const className = event.target.getAttribute("class");
    if (
      wrapperRefMember.current &&
      !wrapperRefMember.current.contains(event.target) &&
        ...
        {tasks?.map((task, index) => (
          ...
          >
            ...
            <div className={styles["task-content"]}>
              <span onClick={() => Router.replace(`/main/tasks/${task.id}`)}>
                {task.name}
              </span>
              <div className={styles.detail}>
                ...
                <span>{`${
                  task.endDate
                    ? format(new Date(task.endDate), "EEE MMM d")
                    : "No date"
                }`}</span>
              </div>
            </div>
            ...
            >
              <div
                className="member-icon"
                onClick={(e) => setVisibleMemberIndexAct(e, index)}
              >
                <UserPhoto
                  ...
                  photoUrl={
                    task.users[0] &&
                    (task.users[0].profilePhoto || Utilities.memberProfile)
                  }
                  tooltipId={`user-task-${task.id}`}
                  tooltipText={task.users[0]?.name}
                />
              </div>
              <img
                className="schedule-icon"
                src={Navigation.schedulePrimary}
