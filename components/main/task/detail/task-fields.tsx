...
import { ItemFields, ProjectTypes, Utilities } from "../../../../assets";
...
const TaskFields = ({
  editableFields: { endDate, project, description, tags, users, userId },
  addTag,
  removeTag,
  editFields,
  replaceTaskAssigned,
    ...
    <div className={styles.container}>
      <div className={styles.field}>
        <ItemFieldWrapper
          ...
          overrideIcon={true}
          OverrideIconComp={() => (
            <UserPhoto photoUrl={users[0]?.profilePhoto} sizePx={45} />
          )}
        >
          <ToggleableAbsoluteWrapper
            ...
            Wrapper={({ children }) => (
              <>
                {users[0] ? (
                  ...
                  <span className={styles["no-assign"]}>
                    {"No Member Assigned"}
                  </span>
                )}
                {children}
            ...
            Content={() => (
              <SearchableUserList
                onUserSelected={handleAssignedChange}
                selectedList={users.map((colab) => colab.id)}
              />
            )}
            ...
            <div>
              <Select
                ...
                value={{
                  ...
                  label: format(new Date(endDate), "hh:mm a"),
                }}
      ...
      <div className={styles.field}>
        <ItemFieldWrapper title="Project" image={ProjectTypes.project}>
          <span>{project?.name}</span>
          {activeInput === "project" ? (
            <div className={styles["tag-select"]}>
              <Select
                options={inputProjects.map((project) => ({
                  ...project,
                  label: project.name,
                  value: project.id,
                }))}
            ...
            <div
              ...
              onClick={() => toggleActiveInput("project")}
            >
      ...
      <div className={styles.field}>
        <ItemFieldWrapper title="Tags" image={ItemFields.tag}>
          ...
          {activeInput === "tags" ? (
            ...
            <div
              className={styles.add}
              onClick={() => toggleActiveInput("tags")}
            >
              <img src={Utilities.add} />
              <span>Add Tag</span>
            </div>
