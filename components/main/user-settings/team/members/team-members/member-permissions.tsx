...
import { IUserPermission } from "../../../../../../interfaces/user/user";
...
const MemberPermissions: React.FC<MemberPermissionsProps> = ({
  ...
  const filter = (data) => {
    ...
    if (!plan || plan?.type === "dam") {
      permissionsList = data.filter(
        (damPermission) => damPermission.category === "Asset Library"
      );
    }
    ...
    if (listOnly || (memberPermissions.length > 0 && permissions.length > 0)) {
      ...
      permissions.forEach((permission) => {
        ...
        const permissionObj = {
          ...permission,
          enabled:
            memberPermissions?.findIndex((memberPer) => memberPer.id === id) !==
            -1,
        };
        if (groups[category]) {
  ...
  const togglePermission = (permission) => {
    const permissionIndex = memberPermissions.findIndex(
      (memberPer) => memberPer.id === permission.id
    );

    // Check if any fields need to be switched to off due to this toggle
    const toggleOffPermissionIndex = permission.toggleOffFields
      ? memberPermissions.findIndex((memberPer) =>
          permission.toggleOffFields.split(",").includes(memberPer.id)
        )
      : -1;
  ...
  return (
    <div
      className={`${styles.container} ${listOnly ? styles["flex-row"] : ""}`}
    >
