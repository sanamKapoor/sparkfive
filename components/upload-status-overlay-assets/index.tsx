...
const UploadStatusOverlayAssets = ({ closeOverlay }) => {
  ...
  const [failUploadingAssets, setFailUploadingAssets] = useState(
    uploadingAssets.filter((asset) => asset.status === "fail")
  );

  const selectedAssets = failUploadingAssets.filter(
    (asset) => asset.isSelected
  );
  let totalSelectAssets = failUploadingAssets.length;

  const toggleSelected = (asset) => {
    const index = uploadingAssets.findIndex(
      (assetItem) => assetItem.asset.name === asset.name
    );

    setUploadingAssets(
      update(uploadingAssets, {
        [index]: {
  ...
  const selectAll = () => {
    setFailUploadingAssets(
      failUploadingAssets.map((assetItem) => ({
        ...assetItem,
        isSelected: true,
      }))
    );
    ...
    <div className={`app-overlay search-container`}>
      ...
      <div className={"search-content"}>
        ...
        <div className={styles.operations}>
          ...
          <span className={styles["select-only-shown-items-text"]}>
            {uploadingAssets.length - failUploadingAssets.length} out of{" "}
            {uploadingAssets.length} successfully uploaded.{" "}
            {failUploadingAssets.length} errors
          </span>
        </div>
