...
const TypeBadge = ({
  ...
  return (
    <div
      onClick={hoverOnMobile}
      ref={projectRef}
      className={`${projectTask && styles["hover-task"]} ${
        projectTask && styles[showProjectTask1]
      } ${styles[type]} ${styles.container} ${
        isMultiple && styles.multiple
      } type-badge`}
    >
      <img
        src={
          socialChannel ? ProjectTypeChannel[socialChannel.toLowerCase()] : icon
        }
      />
      <div className={`${styles.name} name`}>
        ...
        <div
          ...
          className={`${styles["project-task"]} ${
            projectTask && styles[showProjectTask2]
          }`}
        >
          <img src={projectTypeIcon} />
          <p>{projectName}</p>
        </div>
      </div>
