...
const CreditCardForm = ({
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (country.code)
        paymentDetails.billing_details.address.country = country.code;
      if (data.city) paymentDetails.billing_details.address.city = data.city;
      if (state.label)
        paymentDetails.billing_details.address.state = state.label;
      if (data.zip)
        paymentDetails.billing_details.address.postal_code = data.zip;
      if (data.address)
        paymentDetails.billing_details.address.line1 = data.address;

      const method = await stripe.createPaymentMethod(paymentDetails);
      if (method.error) {
        setSubmitError(
          `We could not process your payment: ${method.error.message}`
        );
      } else {
      ...
      console.log(err);
      setSubmitError(
        "We could not process your payment, please choose a different payment method"
      );
    }
  };
  ...
  return (
    <section
      className={`${styles.container} card-content ${
        noBottomMargin && styles["no-bottom"]
      }`}
    >
      <form onSubmit={handleSubmit(onSubmit)} className={styles.form}>
        <div>
          <FormInput
            labId="name-form"
            label="Name on Card"
            InputComponent={<Input type="text" id="name-form" />}
            name="name"
            control={control}
        ...
        <div>
          ...
          <div className={styles["stripe-elem-wrapper"]}>
            <CardNumberElement options={elemOptions} />
          </div>
        </div>
        ...
        <div className={styles["expiry-cvc"]}>
          <div className={styles["stripe-elem-expiry"]}>
            ...
            <div className={styles["stripe-elem-wrapper"]}>
              <CardExpiryElement options={elemOptions} />
            </div>
          </div>
          ...
          <div className={styles["stripe-elem-security"]}>
            ...
            <div className={styles["stripe-elem-wrapper"]}>
              <CardCvcElement options={elemOptions} />
            </div>
          </div>
        </div>
        <div>
          <FormInput
            labId="address-form"
            label="Address"
            InputComponent={<Input type="text" id="address-form" />}
            name="address"
            control={control}
        ...
        <div className={styles["fields-pair"]}>
          <div>
            <FormInput
              labId="city-form"
              label="City"
              InputComponent={<Input type="text" id="city-form" />}
              name="city"
              control={control}
