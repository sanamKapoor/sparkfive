...
const AssetApplication = ({ assetImg, extension, onList = false, bulkSize = false, onClick }) => {
  return (
    <>
      {extension === 'pdf' ? (
        !assetImg ? (
          ...
          : (
          <img onClick={onClick} src={assetImg} className={`${styles.asset} ${onList && styles.small} ${bulkSize && styles['bulk-size']}`} />
        )
      ) : extension === 'doc' ||
        extension ===
          'docx' ? (
