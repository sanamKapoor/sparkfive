...
export default function AssetRelatedFileUpload({ assets: assetData  = [], associateFileId, onUploadFinish = (assets) => {}, currentRelatedAssets = []}){
    ...
    return <>
        {!uploading && <div className={styles['modal-upload-content']}>
        ...
        {uploading && <div className={styles['confirm-modal']}>
            <h2>Add ({assets.length}) Related Files</h2>
            {assets.map(({asset, status}, index)=>{
                return <div className={styles['file-progress']}>
                    <span className={styles['file-name']}>{asset?.name}</span>
                    {status !== "queued" && status !== "fail" && <span className={styles[getBadgeClassByStatus(status)]}>{status}</span>}
                    ...
                    />}
                    {status === "fail" && <span className={styles.fail}>
                                    <span className={styles['fail-badge']}>Fail</span>
                                    <span className={styles['reload-btn']} onClick={()=>{reuploadAsset(index)}}>Reload</span>
                                </span>}
                </div>
            })}
        </div>}
