...
const TaskInput = ({
  ...
  useEffect(() => {
    ...
    setNextWeek(
      startOfWeek(new Date(nextWeek.setDate(nextWeek.getDate() + 8)), {
        weekStartsOn: 1,
      })
    );
  }, []);
  ...
  const handleUserSelection = (user) => {
    if (user.id === selectedUser?.id) setSelectedUser(undefined);
      ...
      setSelectedUser({
        ...user,
        profilePhoto: user.profilePhoto || Utilities.memberProfile,
      });
  ...
  return (
    <div className={`${styles.container}`}>
      ...
      />
      <span className={styles.selected}>{date && format(date, "MMM d")}</span>
      <span className={styles["date-icon"]} onClick={toggleTimeVisible}>
        <UserPhoto
          ...
          sizePx={20}
          tooltipId={"task-input"}
          tooltipText={selectedUser?.name}
        />
      </span>
        ...
        <div className={styles.time}>
          <ul>
            ...
            <li onClick={() => handleDateSeletion(tomorrow)}>
              <span>Tomorrow</span>
              <span>{format(tomorrow, "EEE")}</span>
            </li>
            <li onClick={() => handleDateSeletion(nextWeek)}>
              <span>Next Week</span>
              <span>{format(nextWeek, "EEE")}</span>
            </li>
