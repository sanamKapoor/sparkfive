...
const Notification = () => {
  ...
  const initiateSubscription = async () => {
    try {
      ...
      const {
        data: { initheaders, subHeaders, subData },
      } = await notificationApi.getSubscriptionAuthData();
      appSyncSubscription.initConnection(
        {
          ...
          queryParams: {
            ...
            header: btoa(
              JSON.stringify({
                ...initheaders,
                "Content-Length": initheaders["Content-Length"].toString(),
              })
            ),
        ...
        0,
        () => initListening({ subHeaders, subData })
      );
    } catch (err) {
      console.log(err);
    }
  ...
  const onClear = async (notification) => {
    try {
      await notificationApi.patchNotification({
        notifications: [{ ...notification, status: "cleared" }],
      });
